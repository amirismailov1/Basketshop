...
const Order = () => {
    const {cart,deleteShoesFromCart,postOrders,setCart} = useContext(CustomContext);
...
<h2 className={styles.title}>Заказ</h2>
            ...
            <div className={styles.row}>
                <form action="" className={styles.form} onSubmit={handleSubmit(createOrder)}>
                    ...
                    <span className={styles.error}>{errors?.city && errors?.city?.message}</span>
                    <div className={styles.formItem}>
                    ...
                    <div className={styles.formItem}>
                        ...
                        <textarea {...register('info')} className={styles.formTextArea} name='info'/>
                    </div>
                    <div className={styles.formItem}>
                        <label className={styles.formLabel} htmlFor="pay">Способ оплаты *</label>
                        <select {...register('pay')}  id="" className={styles.formSelect}>
                    ...
                    <div className={styles.formItem}>
                        ...
                        })} className={styles.formInput} type="number" name='money'/>
                    </div>
                    <span>{errors?.money && errors?.money?.message}</span>
                    <div className={styles.formItem}>
                    ...
                    <span>{errors?.check && errors?.check?.message}</span>
                ...
                <div className={styles.right}>
                            ...
                            cart.map(el=>(
                                <li className={styles.item} key={el.id}>
                                    ...
                                    <div className={styles.itemCenter}>
                                        ...
                                        <p className={styles.itemPrice}>{el.price.toString().length > 3? `${el.price.toString().slice(0,-3)},${el.price.toString().substr(-3,)}` : el.price} руб</p>
                                    </div>
                                    <button  type='button' className={styles.itemBtn} onClick={()=>deleteShoesFromCart(el.id)}><svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className={styles.cartFooter}>
                        <div className={styles.cartFooterItem}>
                            ...
                            <p className={styles.itemPrice}>{cart.reduce((acc,rec)=> acc+rec.price,0)}</p>
                        </div>
                        <div className={styles.cartFooterItem}>
                            <p className={styles.cartFooterTitle}>Налог 5%: </p>
                            <div className={styles.cartFooterLine}/>
                            <p className={styles.itemPrice}>{Math.ceil(cart.reduce((acc,rec)=> acc+rec.price,0) / 100 *5)}</p>
                        </div>
                    </div>
                </div>
            </div>
